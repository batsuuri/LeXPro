...
namespace RBACDemo.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        [HttpPost]
        public ActionResult UserCreate(RegisterViewModel model)
        {
            int _retVal = -1;
            if (ModelState.IsValid)
            {
                ApplicationUserManager userMngr = HttpContext.GetOwinContext().GetUserManager<ApplicationUserManager>();
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, Firstname = model.Firstname, Lastname = model.Lastname, PhoneNumber = model.Mobile, TwoFactorEnabled = false };
        ...
        [HttpPost]
        public ActionResult RoleEdit(ApplicationRole _role)
        {
